...
ngx_http_init_phase_handlers(ngx_conf_t *cf, ngx_http_core_main_conf_t *cmcf)
{
    ...
    for (i = 0; i < NGX_HTTP_LOG_PHASE; i++) {
        ...
        case NGX_HTTP_PREACCESS_PHASE:
             checker = ngx_http_core_preaccess_phase;
             break;

        case NGX_HTTP_POST_READ_PHASE:
            checker = ngx_http_core_post_read_phase;
            break;
        }
...
static void ngx_http_dump_location(ngx_http_core_loc_conf_t *clcf)
{
    if (clcf == NULL) {
        return;
    }
    ...
    printf("[%p] : ", clcf);
    printf("name : %s",clcf->name.len ? clcf->name.data : "null");
    printf(" noname:%d lmt_excpt:%d named:%d exact_match:%d noregex:%d\n",
        clcf->noname, clcf->lmt_excpt, clcf->named,
